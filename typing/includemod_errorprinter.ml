...
and functor_params ~expansion_token ~env ~before ~ctx
    ({got;expected;symptom} : _ Err.diff) =
  match got.Err.params, expected.Err.params with
  | [], _ -> functor_expected ~before ~ctx
  | _, [] ->
      unexpected_functor ~env:env.i_env ~before ~ctx
        { Err.got; expected; symptom }
  | _ :: _, _ :: _ ->
  let is_modal = Is_modal.functor_params_symptom symptom in
  let d = Functor_suberror.Inclusion.patch env
      (got.params, got.res) (expected.params, expected.res) in
