...
open Ctype
open Longident
let noloc x = Location.mknoloc x
open Path
open Asttypes
open Types
open Mode
...
let () = Mode.print_longident := longident
...
let expand_longident_head name =
  ...
  | None -> None
  | Some i ->
    let noloc x = Location.mknoloc x in
    Some
      (Ldot
        (noloc (Lident (String.sub name 0 i)),
          noloc (Unit_info.modulize
...
let rec rewrite_double_underscore_longidents env (l : Longident.t) =
  match l with
  | Ldot (l, s) ->
    Ldot ({ l with txt = rewrite_double_underscore_longidents env l.txt }, s)
  | Lapply (a, b) ->
    Lapply ({ a with txt = rewrite_double_underscore_longidents env a.txt },
            { b with txt = rewrite_double_underscore_longidents env b.txt })
  | Lident name ->
    match expand_longident_head name with
    | None -> l
    | Some l' ->
...
let rec lid_of_path = function
  ...
  | Path.Pdot (p1, s) | Path.Pextra_ty (p1, Pcstr_ty s)  ->
      Longident.Ldot (noloc (lid_of_path p1), noloc s)
  | Path.Papply (p1, p2) ->
      Longident.Lapply (noloc (lid_of_path p1), noloc (lid_of_path p2))
  | Path.Pextra_ty (p, Pext_ty) -> lid_of_path p
  | Path.Pextra_ty (p, Punboxed_ty) ->
    match p with
    | Pident id -> Longident.Lident (Ident.name id ^ "#")
    | Pdot (p, s) -> Longident.Ldot (noloc (lid_of_path p), noloc (s ^ "#"))
...
type type_or_scheme = Out_type.type_or_scheme = Type | Type_scheme
...
module Names : sig
  ...
  val refresh_weak : unit -> unit [@@warning "-32"]
end = struct
...
let rec tree_of_modal_typexp mode modal ty =
  ...
  let pr_typ alloc_mode =
    ...
    | Tpackage { pack_path = p; pack_cstrs = fl } ->
        ...
        Otyp_module { opack_path = tree_of_path (Some Module_type) p;
                      opack_cstrs = fl }
    | Tof_kind jkind ->
...
let tree_of_label l =
  let olab_mut, olab_atomic =
    ...
    | Mutable { atomic; _ } ->
        let atomic : Asttypes.atomic_flag = match atomic with
          | Atomic -> Atomic | Nonatomic -> Nonatomic
        in
        Asttypes.Mutable, atomic
    | Immutable -> Asttypes.Immutable, Asttypes.Nonatomic
  in
  let olab_modalities = tree_of_modalities l.ld_mutable l.ld_modalities in
  { Outcometree.olab_name = Ident.name l.ld_id;
    olab_mut;
    olab_atomic;
...
let _add_type_decl_to_preparation id decl =
...
let _tree_of_prepared_type_declaration id decl rs =
...
let _prepared_extension_constructor id ppf ext =
...
let strings_of_paths namespace p =
  let trees = List.map (Out_type.namespaced_tree_of_path namespace) p in
...
let rec prepare_class_type params = function
  ...
  | Cty_signature sign ->
      ...
      Meths.iter (fun _lab (priv, _virt, ty) ->
        let ty, _ = method_type priv ty in
        prepare_type ty) sign.csig_meths
  | Cty_arrow (_, ty, cty) ->
      prepare_type ty;
...
let _class_declaration id ppf cl =
...
let _cltype_declaration id ppf cl =
...
let rec _functor_parameters ~sep custom_printer = function
  | [] -> ignore
  | [id,param] ->
      Fmt.dprintf "%t%t"
        (custom_printer param)
        (_functor_param ~sep ~custom_printer id [])
  | (id,param) :: q ->
      Fmt.dprintf "%t%a%t"
        (custom_printer param)
        sep ()
        (_functor_param ~sep ~custom_printer id q)
and _functor_param ~sep ~custom_printer id q =
  match id with
  | None -> _functor_parameters ~sep custom_printer q
  | Some id ->
      Naming_context.with_arg id
        (fun () -> _functor_parameters ~sep custom_printer q)
...
type 'variety trace_format =
  | Unification : Errortrace.unification trace_format [@warning "-37"]
  | Equality    : Errortrace.comparison  trace_format [@warning "-37"]
  | Moregen     : Errortrace.comparison  trace_format [@warning "-37"]
...
let _trees_of_type_path_expansion (tp,tp') =
...
let explanation (type variety) intro prev env
  ...
  | Errortrace.Function_label_mismatch _ -> None
  | Errortrace.Tuple_label_mismatch _ -> None
  | Errortrace.First_class_module _ -> None
...
let _error trace_format mode subst env tr txt1 ppf txt2 ty_expect_explanation =
  reset ();
  ...
  let jkind_error = match Misc.last tr with
    ...
    | Some (Diff _ | Escape _ | Variant _ | Obj _ | Incompatible_fields _
           | Rec_occur _ | Function_label_mismatch _ | Tuple_label_mismatch _
...
let _abbreviate ~abbrev f =
  f ?abbrev:(if abbrev then Some (Abbrev.abbrev ()) else None)

let _tree_of_path = tree_of_path None
let _tree_of_module ident ?(ellipsis = false) =
  tree_of_module ident ?abbrev:(if ellipsis then Some (Abbrev.ellipsis ()) else None)
let _tree_of_signature sg = tree_of_signature sg
let _tree_of_modtype ?(abbrev = false) ty =
  _abbreviate ~abbrev tree_of_modtype ty
let _tree_of_modtype_declaration ?(abbrev = false) id md =
  _abbreviate ~abbrev tree_of_modtype_declaration id md
let type_expansion mode ppf ty_exp =
  type_expansion ppf (trees_of_type_expansion mode ty_exp)
let _tree_of_type_declaration ident td rs =
  with_hidden_items [{hide=true; ident}]
    (fun () -> tree_of_type_declaration ident td rs)

(* Printers for declarations *)
let value_description id ppf decl =
  !Oprint.out_sig_item ppf (tree_of_value_description id decl)

let modtype_declaration id ppf md =
  !Oprint.out_sig_item ppf (tree_of_modtype_declaration id md)

let class_declaration id ppf decl =
  !Oprint.out_sig_item ppf (tree_of_class_declaration id decl Trec_first)

let cltype_declaration id ppf decl =
  !Oprint.out_sig_item ppf (tree_of_cltype_declaration id decl Trec_first)

(** Doc module: Format_doc printers *)
module Doc = struct
...
(** Top-level include: Format.formatter printers for compatibility *)
let wrap_printing_env = wrap_printing_env
let longident = Fmt.compat longident
let ident = Fmt.compat ident
let path : Path.t Format_doc.format_printer = Fmt.compat path
...
let type_expansion mode = Fmt.compat (type_expansion mode)
...
let extension_constructor id = Fmt.compat (extension_constructor id)
let extension_only_constructor id = Fmt.compat (extension_only_constructor id)
let value_description id = Fmt.compat (value_description id)
let type_declaration id = Fmt.compat (type_declaration id)
let modtype_declaration id = Fmt.compat (modtype_declaration id)
let class_declaration id = Fmt.compat (class_declaration id)
let cltype_declaration id = Fmt.compat (cltype_declaration id)
let modtype = Fmt.compat modtype
let signature = Fmt.compat signature
let class_type = Fmt.compat class_type
let modality ?id ax =
  let id = Option.map (fun f -> Fmt.deprecated f) id in
  Fmt.compat (modality ?id ax)
let raw_type_expr = Fmt.compat raw_type_expr
let raw_row_desc = Fmt.compat raw_row_desc
let raw_field = Fmt.compat raw_field
