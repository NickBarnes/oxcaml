...
(* Represents information about an array type inferred using type-directed
   disambiguation. *)
type array_info =
  { ty_elt : type_expr option [@warning "-69"];
    mut : mutable_flag } [@@warning "-69"]

let _disambiguate_array_literal ~loc env expected_ty =
  ...
  | Ppat_var name ->
      ...
      let desc =
        { val_type = expected_ty;
          val_kind = Val_reg Jkind.Sort.(of_const Const.for_predef_value);
...
let unify_pat_types_refine ~refine loc penv ty ty' =
  ignore (unify_pat_types_return_equated_pairs ~refine loc penv
            ~pat:ty ~expected:ty')
...
let continuation_variable = function
  ...
  | Some (id, (desc:Types.value_description)) ->
    [{pv_id = id;
     ...
     pv_sort = Jkind.Sort.(of_const Const.for_predef_value);
     pv_uid= desc.val_uid}]
...
let _iter_pattern_variables_type_mut ~f_immut ~f_mut pvs =
...
let solve_constructor_annotation
  ...
  let ids_decls =
    List.map
      (fun (name, jkind_annot_opt) ->
        ...
        let decl =
          new_local_type ~loc:name.loc
            ~manifest_and_scope:(tv, Ident.lowest_scope) Definition jkind in
        let (id, new_env) =
          Env.enter_type ~scope:expansion_scope name.txt decl !!penv in
        Pattern_env.set_env penv new_env;
        ({name with txt = id}, jkind_annot_opt, decl, tv))
      name_list
  in
  ...
  in
  let existentials =
    List.map (fun (x, jk, _, _) -> (x, jk)) ids_decls
  in
  if ids_decls <> [] then begin
    let ids_decls =
      List.map (fun (x, _, d, tv) -> (x.txt, (d, tv))) ids_decls
    in
...
let solve_Ppat_construct tps (penv : Pattern_env.t) loc constr no_existentials
  ...
  let ty_args, equated_types, existential_ctyp =
    with_local_level_generalize_structure begin fun () ->
      ...
      let ty_args, _ty_args_ty, ty_res, equated_types, existential_ctyp =
...
let solve_Ppat_record_field ~refine:_ loc penv label label_lid record_ty
      record_form =
  with_local_level_generalize_structure begin fun () ->
...
let type_for_loop_like_index ~error ~loc ~env ~param ~any ~var =
  ...
  | Ppat_any ->
    any (Ident.create_local "_for", Uid.mk ~current_unit:(Env.get_current_unit ()))
  | Ppat_var name ->
      var ~name
          ~pv_mode:Value.(min |> disallow_right)
          ...
          ->
            ...
            let pv =
              { pv_id; pv_uid; pv_mode;
                pv_value_kind = Val_reg Jkind.Sort.(of_const Const.for_loop_index);
                pv_type; pv_loc;
                pv_kind = Std_var;
                pv_attributes;
...
and type_pat_aux
  ...
  = fun tps category ~no_existentials ~alloc_mode ~mutable_flag ~penv sp
        expected_ty sort ->
  assert (penv.Pattern_env.in_counterexample = false);
  ...
  | Ppat_interval (l, r) ->
      ...
      | Const_char c1, Const_char c2 ->
        expand_interval (Char.code c1) (Char.code c2)
          ~make:(fun i -> Ast_helper.Const.char ~loc (Char.chr i))
      | Const_untagged_char c1, Const_untagged_char c2 ->
        expand_interval (Char.code c1) (Char.code c2)
          ~make:(fun i ->
            { pconst_desc = Pconst_untagged_char (Char.chr i);
              pconst_loc = loc })
      | _ ->
...
let type_class_arg_pattern cl_num val_env met_env l spat =
  ...
  let (pv, val_env, met_env) =
    List.fold_right
      (fun {pv_id; pv_uid; pv_type; pv_loc; pv_kind; pv_value_kind = _;
            pv_attributes; pv_sort}
        (pv, val_env, met_env) ->
         let check s =
           if pv_kind = As_var
...
let rec check_counter_example_pat
  ...
  | Tpat_construct(cstr_lid, constr, targs, _) ->
      ...
      let (ty_args, existential_ctyp) =
        solve_Ppat_construct type_pat_state penv loc constr None None
          expected_ty
      in
      map_fold_cont
...
and type_expect_
      ...
      let (pat_exp_list, body, _new_env) =
        ...
        *)
        ...
        ~before_generalize:(fun (_pat_exp_list, body, new_env) ->
          ...
          unify_exp ~sexp new_env body (newvar (Jkind.Builtin.any ~why:Dummy_jkind)))
      in
      let exp =
        ...
        if TypeSet.mem ty seen then false else
          match get_desc ty with
            Tarrow (_l, ty_arg, ty_fun, _com) ->
              (try Ctype.unify_var env (newvar2 outer_level (Jkind.Builtin.any ~why:Dummy_jkind)) ty_arg
  ...
  | Pexp_pack (m, optyp) ->
      ...
      | Some ptyp ->
        ...
        let pty, cty = type_constraint env t Alloc.Const.legacy in
        begin match get_desc (instance pty) with
          | Tpackage pack ->
            ...
            rue {
              ...
              exp_loc = loc; exp_extra = [Texp_constraint cty, loc, []];
  ...
  | Pexp_extension ({ txt = ("probe_is_enabled"
      ...
      | PStr ([{ pstr_desc =
                   Pstr_eval
                     ({pexp_desc=(Pexp_constant {pconst_desc=Pconst_string(name,_,None); _});
                       pexp_loc = name_loc;
                       _ } ,
                      _)}]) ->
        check_probe_name name name_loc env;
...
and _solve_Pexp_field ~label_usage env sexp srecord lid =
  let (record, _, _, label, _, _) =
  ...
  *)
  let (vars, arg) =
    with_local_level_generalize begin fun () ->
      ...
      in
      (vars, type_argument ~overwrite env arg_mode sarg ty_arg (instance ty_arg))
    end
    ~before_generalize:(fun (_,arg) -> may_lower_contravariant env arg)
  in
  if vars <> [] then check_univars env "field value" arg label.lbl_arg vars;
...
(*REMOVEBLOCK3 and type_application_REMOVED env funct sargs =
  ...
  | Arg (Known_arg { sarg; ty_arg; ty_arg0; wrapped_in_some }) ->
      let arg =
        if wrapped_in_some then
          ...
          type_argument_REMOVED env sarg ty_arg ty_arg0 REMOVEBLOCK3*)
      in
...
and type_application_REMOVED env funct sargs = ignore (env, funct, sargs)
RMBLOCK4*)
  let is_ignore funct =
...
and type_construct ~overwrite env (expected_mode : expected_mode) loc lid sarg
  ...
  let separate = !Clflags.principal || Env.has_local_constraints env in
  let unify_as_construct _ty_expected =
    with_local_level_generalize_structure_if separate begin fun () ->
  ...
  if not separate then
    unify_exp_types loc env texp.exp_type (instance ty_expected);
  let recarg =
   ...
   (* propagation of the argument *)
    with_local_level_generalize begin fun () ->
      ...
      let half_typed_cases =
        map_conts
        (fun ({ Parmatch.pattern; _ } as untyped_case, case_data) _cont ->
          let htc =
...
and type_cases
  ...
  *)
  map_half_typed_cases ?conts category env pat_mode ty_arg sort_arg ty_res loc caselist
    ~check_if_total
    ~type_body:begin
      fun { pc_guard; pc_rhs } pat ~when_env:_ ~ext_env ~cont ~ty_expected
...
and _type_effect_cases
  ...
  = fun category env ty_res_explained loc caselist conts ->
      ...
      let _ = newvar (Jkind.Builtin.any ~why:Dummy_jkind) in
      (* remember original level *)
      with_local_level begin fun () ->
        (* Create a locally type abstract type for effect type. *)
        let new_env, ty_arg, ty_cont =
          let decl = Ctype.new_local_type ~loc Definition (Jkind.Builtin.value ~why:Existential_type_variable) in
          let scope = create_scope () in
        ...
        let cases, _ = type_cases category new_env
          (simple_pat_mode Value.legacy) (mode_return Value.legacy)
          ty_arg Jkind.Sort.value
...
and type_let ?check ?check_strict ?(force_toplevel = false)
  ...
  let (pat_list, exp_list, new_env, mvs, sorts, _pvs) =
    ...
    ~before_generalize: begin fun (mode_pat_typ_list, exp_list, _, _, _, _pvs) ->
      List.iter2
        (fun (_, pat, _) (exp, _) ->
  ...
  if is_recursive then
    List.iter
      (fun {vb_pat=pat} -> match pat.pat_desc with
           Tpat_var _ -> ()
         | Tpat_alias ({pat_desc=Tpat_any}, _, _, _, _, _, _) -> ()
...
let _quoted_constr = Style.as_inline_code Pprintast.Doc.constr

let _tuple_component_unused ~print_article ppf lbl =
  let article =
...
let _report_this_function ppf _funct =
  Format.fprintf ppf "This function"
...
let [@warning "-32"] msg = Fmt.doc_printf
      ...
      Location.error_of_printer ~loc (fun ppf () ->
        Printtyp.wrap_printing_env ~error:true env (fun () ->
          ...
          Option.iter (fun hint ->
            Format_doc.fprintf ppf "@ %a" Format_doc.pp_doc hint)
      ...
      Location.error_of_printer ~loc (fun ppf () ->
        Errortrace_report.ambiguous_type ppf env tp tpl
          (msg "The %s %a@ belongs to the %s type"
               name (Style.as_inline_code Printtyp.longident) lid
              type_name)
          (msg "The %s %a@ belongs to one of the following %s types:"
               name (Style.as_inline_code Printtyp.longident) lid type_name)
    ...
    let sub =
      ...
      | Constructor name ->
        ...
        [ Location.msg "@[Hint: All arguments of the constructor %a@\n\
          ...
          (Style.as_inline_code Printtyp.longident) name ]
