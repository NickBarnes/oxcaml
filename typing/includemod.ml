...
and try_modtypes ~direction ~loc env subst ~modes
  ...
  | Mty_functor (param1, res1, mres1), Mty_functor (param2, res2, mres2) ->
      ...
      | _, Error {Error.symptom = Error.Functor Error.Params res; _} ->
          let got = res.got in
          let expected = res.expected in
          let d = Error.diff
              { Error.params = force_functor_parameter param1::got.Error.params;
                res = got.res }
              { Error.params = force_functor_parameter param2::expected.Error.params;
                res = expected.res }
              res.symptom
          in
      ...
      | Error symptom, _ ->
          ...
          in
          let d = Error.diff
            { Error.params = force_functor_parameter param1::params1; res = res1 }
            { Error.params = force_functor_parameter param2::params2; res = res2 }
            (Error.Param symptom)
          in
          Error Error.(Functor (Params d))
      | Ok _, Error res ->
  ...
  | _ ->
    ...
    | None ->
        ...
        | _, Mty_functor _ ->
            ...
            in
            let d = Error.diff
              { Error.params = params1; res = res1 }
              { Error.params = params2; res = res2 }
              Error.Incompatible in
            Error Error.(Functor (Params d))
...
let modtypes_consistency ~loc env mty1 mty2 =
  let direction = Directionality.unknown ~mark:false in
  match modtypes ~direction ~loc env
          Subst.identity ~modes:All mty1 mty2 Shape.dummy_mod
  with
  | Ok _ -> ()
  | Error reason -> raise (Error (env, Error.(In_Module_type reason)))
