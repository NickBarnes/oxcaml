      ...
      let delayed_jkind_checks =
        ...
        | Asttypes.Recursive ->
          List.map2
            ...
            ids_list sdecl_list
      in
      (tdecls, decls, new_env, delayed_jkind_checks)
    end
  in
...
let transl_package_constraint ~loc _env ty =
  { type_params = [];
    type_arity = 0;
...
(* Upstream helper functions for error reporting *)
...
let report_error ~loc = function
             ...
             | Type_declaration { id ; decl ; unboxed_version } ->
                 ...
                 fprintf ppf "@[<v>%s@;<1 2>%a@;%s"
                   ...
                   (Style.as_inline_code @@
                    Out_type.prepared_type_declaration id)
                   decl
                   post
             | Gadt_constructor c ->
  ...
  | Jkind_empty_record ->
    Location.errorf ~loc
      "Records must contain at least one runtime value."
