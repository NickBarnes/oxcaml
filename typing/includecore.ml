...
let child_close_over_coercion_opt id (c : held_locks option) : held_locks option =
  match c with
  | None -> None
  | Some (locks, lid, loc) ->
    let noloc x = Location.mknoloc x in
    Some (locks, Longident.Ldot (noloc lid, noloc id), loc)
  ...
  | (Val_prim p, _) ->
      let pc =
        { pc_desc = p; pc_type = vd2.Types.val_type;
          pc_poly_mode = None; pc_poly_sort = None;
