...
code. Reasonable usage includes, for example, copy-editing text,
writing assistance for people with foreign-language or writing
...
hallucinated reports of security issues, or experiments on automated
interactions with human contributors without explicitly obtaining
...
1. We strive to evaluate proposed contributions based on their merit,
   not their authors -- humans or programs.

2. We ask that you have either written or read and reviewed yourself
   every part of your contribution. You are responsible for its
   quality.

3. If a significant portion of your code, PR description, review or
   message on this repository has been entirely or almost-entirely
   authored by an automated tool, we ask that you disclose that the
   patch/description/review/message was produced by automation rather
   than yourself, mentioning which tool was used and for
   what. (For example, "The testsuite addition were mostly generated
   by Copilot.", or "This PR description was generated by ChatGPT from
   a short summary.")

   This demand applies even if you reviewed the contribution or
   performed lightweight copy-editing -- but not if the tool was only
   used to review or slightly modify human-produced content. One way
   to think of it is as follows: if another human had been involved
   rather than an automated tool, and your integration of their
   contribution could be qualified as plagiarism, then you should
   disclose their involvement.

4. You should be aware that ensuring the quality of AI-assisted output
   is difficult, and be willing to do the corresponding work before
   submitting the contribution. Reviewing AI-produced code for quality
   ...
   result in maintainers disregarding your contributions in the
   future.

5. The current position of authors of AI-assisted programming tools is
   that it is their user's responsibility that the produced output
   does not violate copyright by including copyrighted portions of
   a program from their training set. If your contribution includes
   substantial AI-produced parts, you must check whether they are
   derivative works of an existing project and present your
   contribution accordingly. (In particular, you cannot contribute
   this contribution if the license of the original code does not
   allow it.)

6. Please be mindful that reviewer time is a scarce
   resource. Contributions (whether human- or tool-authored) should
